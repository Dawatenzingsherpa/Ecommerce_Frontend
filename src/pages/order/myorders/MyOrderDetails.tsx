...
import { fetchOrderDetails ,cancelOrder} from "../../../store/checkoutSlice"
...
const MyOrderDetails = () => {
  ...
  console.log(orderDetails[0]?.Order?.Payment?.paymentMethod)

  const shippingCost:number = 150
    ...
    <Navbar/>
      <div className="py-1 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
        ...
        <div className="flex justify-start item-start space-y-5 flex-col">
          ...
          <p className="text-base dark:text-gray-300 font-medium leading-6 text-gray-600">{new Date(orderDetails[0]?.Order?.createdAt as string).toLocaleDateString()}</p>
        </div>
            ...
            {orderDetails.length>0 && orderDetails.map((order)=>(
              <div key={order?.id} className="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full" >
                <div className="pb-4 md:pb-8 w-full md:w-40">
                  <img className="w-full hidden md:block" src={order?.Product?.imageUrl} alt="dress" />
                ...
                <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
                  ...
                  <div className="flex justify-between space-x-8 items-start w-full">
                    ...
                    <p className="text-base dark:text-white xl:text-lg font-semibold leading-6 text-gray-800">Rs.{Number(order?.quantity) * Number(order?.Product?.productPrice)} </p>
                  </div>
                </div>
              </div>)
            )}
            ...
            <div className="flex justify-center flex-col md:flex-row flex-col items-stretch w-full space-y-4 md:space-y-0 md:space-x-6 xl:space-x-8">
              <div className="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
                ...
                <div className="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
                  ...
                  <div className="flex justify-between items-center w-full">
                    ...
                    <p className="text-base dark:text-grayorderDetails-300 leading-4 text-gray-600">{orderDetails[0]?.Order?.Payment?.paymentMethod}</p>
                  </div>
                  <div className="flex justify-between items-center w-full">
              ...
              <div className="flex flex-col justify-center px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
                ...
                <div className="flex justify-between items-start w-full">
                  ...
                  <p className="text-lg font-semibold leading-6 dark:text-white text-gray-800">Rs{shippingCost}</p>
                </div>
              </div>
            </div>
          </div>
